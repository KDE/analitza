...
void FunctionTest::initTestCase()
{
	...
	qDebug() << "AKIII EMPIEZA";
	Keomath::Function f(Analitza::Expression("x->x-2"), v);

	Keomath::VectorXd a(1);
	a(0) = 34;

	qDebug() << f.valueCount() << f.errors() << f.evaluate(a)(0);

	QVERIFY(f.evaluate(a)(0) == 32);
